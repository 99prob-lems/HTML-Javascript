...
function getWeatherData(city) { //Once the city name is typed Openweathermap provides JSON data 
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            document.getElementById("map-weather-search-container").style.display = "block";
            ...
            document.getElementById("map-weather-search-container").style.display = "block";
            document.getElementById("errorMessage").innerHTML = "Error: City not found.";
...
function search() {
    ...
    var topcity = document.getElementById("entersite").value;  
    if (cityValidation(city) || cityValidation(topcity)) {
        if(city){
            getWeatherData(city);
            document.getElementById('citiessec').scrollIntoView();
        } else if (topcity) {
            getWeatherData(topcity);
            document.getElementById('citiessec').scrollIntoView();
        } else {
